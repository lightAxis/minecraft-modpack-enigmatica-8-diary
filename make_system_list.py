...
def __parse_name_desc(mdPath: str, mainFileDirPath: str):
    ...
    for line in lines:
        ...
        if (find_name):
            desc_temp.Desc = line
            desc_temp.RelPath = os.path.relpath(mdPath, mainFileDirPath)
            break
...
def generate_system_list(systemMdPath: str, systemMdsPath: str):
    ...
    descs: List[desc_t] = []
    systemMdFolderPath = os.path.split(systemMdPath)[0]
    for filePath in filePaths:
        if filePath == systemMdPath:
            continue
        descs.append(__parse_name_desc(filePath, systemMdFolderPath))
